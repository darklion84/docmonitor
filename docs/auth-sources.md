...
3. **Запись в `config/auth-sources.yaml`**:
   ...
   Если в одном auth-домене несколько страниц, не дублируйте `browser_steps` — заведите
   **`auth_templates`** и ссылайтесь на него полем `auth:`:
   ```yaml
   auth_templates:
     foo:
       browser_steps:
   ...
   sources:
     - id: foo-overview
       title: "Foo overview"
       url: https://foo.example.com/overview/
       tag: foo
       auth: foo                  # ← подтянет browser_steps из шаблона
       time_between_check: { hours: 6 }
     - id: foo-api-reference
       title: "Foo API reference"
       url: https://foo.example.com/api/
       tag: foo
       auth: foo
       time_between_check: { hours: 6 }
   ```
   В source можно указать собственный `browser_steps` — он переопределит шаблон целиком.
...
- **Параллелизм**: на тяжёлых auth-страницах разумно держать
  ...
  чем `FETCH_WORKERS` у CDIO. Сейчас оба = 10. Если массово реквестить
  recheck (`?recheck=1` сразу для 4+ auth-watch'ей) — sockpuppet может упереться
  в `connect_over_cdp Timeout 60000ms` (4 параллельных Chrome'а под Keycloak —
  тяжёлый burst). По штатному расписанию `time_between_check` бёрстов нет.
  Если попали — `docker compose restart browser-sockpuppet-chrome` и проверять
  по одному (в проде проще задать `MAX_CONCURRENT_CHROME_PROCESSES=4`).
