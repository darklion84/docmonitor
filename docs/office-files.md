...
> **Порядок важен: сначала конфиг, потом файл.** Адаптер реагирует на FS-события
> (`watchdog.PollingObserver`) и сверяет путь с записями в yaml. Если положить
> файл *до* того, как он попал в `office-sources.yaml`, событие создания
> отработает «вхолостую» (источник ещё неизвестен), и `.txt` не появится сразу —
> только на ближайшем периодическом проходе (по умолчанию раз в 600с).

1. **Сначала** допишите запись в `config/office-sources.yaml`:
   ```yaml
   sources:
   ...
   ```
   Конфиг смонтирован RO бинд-маунтом — адаптер видит правку сразу, перезапуск
   не нужен.
2. **Потом** положите/пересохраните файл в `office-src/`, например `pricing.docx`.
...
**Если файл уже лежал** (порядок нарушен) — не ждите 600с, варианты:
- пересохраните оригинал (любая запись → новое FS-событие → реакция ~1с), **или**
- дёрните разовый прогон (читает свежий yaml, обработает все источники):
  ```
  docker compose exec -T office-adapter python /src/docmonitor/office_adapter.py
  ```
